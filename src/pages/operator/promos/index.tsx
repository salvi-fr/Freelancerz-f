...
useEffect(() => {
  ...
  return (
    <div>
      <DashboardPageHeader title="All Promos" iconName="box" from="Operator"
      
      button={
        <Link href="/operator/promos/new">
          <a>
...
{ promosData && promosData.length>0? <>
      {promosData.slice(from,to).map((item,ind) => (
          ...
          >
            ...
            <Link href={`/operator/promos/edit/${item._id}`}>
              <Typography
                as="a"
                href={`/operator/promos/edit/${item._id}`}
                color="inherit"
              >
...
PromosList.layout = OperatorDashboardLayout;
