...
const LecturesList = () => {
  ...
  return (
    <div>
      <DashboardPageHeader title="Your Lectures" iconName="box"  from="Operator"
      button={
        <Link href="/operator/lectures/new">
          <a>
...
<>
      {lecturesData.slice(from,to).map((item,ind) => (
          ...
          key={ind}
            // as="a"
            // href={`/operator/lectures/${item._id}`}
            my="1rem"
            padding="15px 24px"
          >
            <div>
            ...
            <Link href={`/operator/lectures/edit/${item._id}`}>
              <Typography
                as="a"
                href={`/operator/lectures/edit/${item._id}`}
                color="inherit"
              >
...
LecturesList.layout = OperatorDashboardLayout;
