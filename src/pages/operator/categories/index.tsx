...
const toggleDialog = useCallback(() => {
  ...
  return (
    <div>
      <DashboardPageHeader title="All Categories" iconName="box" from="Operator"
      
      button={
        <Link href="/operator/categories/new">
          <a>
...
{ categoriesData && categoriesData.length>0? <>
      {categoriesData.slice(from,to).map((item,ind) => (
          ...
          >
            ...
            <Link href={`/operator/categories/edit/${item._id}`}>
              <Typography
                as="a"
                href={`/operator/categories/edit/${item._id}`}
                color="inherit"
              >
...
CategoriesList.layout = OperatorDashboardLayout;
