...
const PaymentsList = () => {
  ...
  return (
    <div>
      <DashboardPageHeader title="Payments" iconName="box"  from="Operator"
      button={
        <Link href="/operator/payments/new">
          <a>
...
<>
      {paymentsData.slice(from,to).map((item,ind) => (
          ...
          key={ind}
            // as="a"
            // href={`/operator/payments/${item._id}`}
            my="1rem"
            padding="15px 24px"
          >
            <div>
            ...
            <Link href={`/operator/payments/edit/${item._id}`}>
              <Typography
                as="a"
                href={`/operator/payments/edit/${item._id}`}
                color="inherit"
              >
...
PaymentsList.layout = OperatorDashboardLayout;
