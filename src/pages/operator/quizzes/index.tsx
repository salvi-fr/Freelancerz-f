...
const handleDelete = async () => {
  ...
  return (
    <div>
      <DashboardPageHeader title="Your Quizzes" iconName="box"  from="Operator"
      button={
        <Link href="/operator/quizzes/new">
          <a>
...
<>
      {quizzesData.slice(from,to).map((item,ind) => (
        // <Link 
        // href={`/operator/quizzes/${item._id}`}
        //  key={ind}>
          <TableRow
          key={ind}
            // as="a"
          ...
          >
            ...
            <Link href={`/operator/quizzes/edit/${item._id}`}>
              <Typography
                as="a"
                href={`/operator/quizzes/edit/${item._id}`}
                color="inherit"
              >
...
QuizzesList.layout = OperatorDashboardLayout;
