...
const ModulesList = () => {
  ...
  return (
    <div>
      <DashboardPageHeader title="Modules" iconName="box"  from="Operator"
      button={
        <Link href="/operator/modules/new">
          <a>
...
<>
      {modulesData.slice(from,to).map((item,ind) => (
          ...
          key={ind}
            // as="a"
            // href={`/operator/modules/${item._id}`}
            my="1rem"
            padding="15px 24px"
          >
            <div>
            ...
            <Link href={`/operator/modules/edit/${item._id}`}>
              <Typography
                as="a"
                href={`/operator/modules/edit/${item._id}`}
                color="inherit"
              >
...
ModulesList.layout = OperatorDashboardLayout;
