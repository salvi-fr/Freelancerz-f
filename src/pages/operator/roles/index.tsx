...
const toggleDialog = useCallback(() => {
  ...
  return (
    <div>
      <DashboardPageHeader title="All Roles" iconName="box" from="Operator"
      
      button={
        <Link href="/operator/roles/new">
          <a>
...
{ rolesData && rolesData.length>0? <>
      {rolesData.slice(from,to).map((item,ind) => (
          ...
          >
            ...
            <Link href={`/operator/roles/edit/${item._id}`}>
              <Typography
                as="a"
                href={`/operator/roles/edit/${item._id}`}
                color="inherit"
              >
...
RolesList.layout = OperatorDashboardLayout;
