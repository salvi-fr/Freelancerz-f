...
const ArticlesList = () => {
  ...
  return (
    <div>
      <DashboardPageHeader title="Your Articles" iconName="box" from="Operator"
        button={
          <Link href="/operator/articles/new">
            <a>
      ...
      {articlesData && articlesData.length > 0 ?
        <>
          {articlesData.slice(from, to).map((item, ind) => (
            // <Link href={`/operator/articles/${item._id}`} key={ind}>
            <TableRow
              // as="a"
              // href={`/operator/articles/${item._id}`}
              key={ind}
              my="1rem"
              padding="15px 24px"
            >
              ...
              <Hidden flex="0 0 0 !important" down={769}>
                ...
                <Typography className="pre" textAlign="center" color="text.muted">
                  <Link href={`/operator/articles/edit/${item._id}`}>
                    <Typography
                      as="a"
                      href={`/operator/articles/edit/${item._id}`}
                      color="inherit"
                    >
...
ArticlesList.layout = OperatorDashboardLayout;
