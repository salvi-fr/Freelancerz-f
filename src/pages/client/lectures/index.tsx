...
const LecturesList = () => {
  ...
  return (
    <div>
      <DashboardPageHeader title="Your Lectures" iconName="box"  from="Instructor"
      button={
...
<>
      {lecturesData.slice(from,to).map((item,ind) => (
          ...
          key={ind}
            // as="a"
            // href={`/instructor/lectures/${item._id}`}
            my="1rem"
            padding="15px 24px"
          >
            <div>
            ...
            <Link href={`/instructor/lectures/edit/${item._id}`}>
              <Typography
                as="a"
                href={`/instructor/lectures/edit/${item._id}`}
                color="inherit"
              >
...
LecturesList.layout = InstructorDashboardLayout;
