...
const EventsList = () => {
  ...
  return (
    <div>
      <DashboardPageHeader title="Events" iconName="box"  from="Instructor"
      button={
        <Link href="/instructor/events/new">
          <a>
...
<>
      {eventsData.slice(from,to).map((item,ind) => (
          ...
          key={ind}
            // as="a"
            // href={`/instructor/events/${item._id}`}
            my="1rem"
            padding="15px 24px"
          >
            <div>
            ...
            <Link href={`/instructor/events/edit/${item._id}`}>
              <Typography
                as="a"
                href={`/instructor/events/edit/${item._id}`}
                color="inherit"
              >
...
EventsList.layout = InstructorDashboardLayout;
