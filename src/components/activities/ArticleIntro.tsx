...
const ArticleIntroCard: React.FC<ArticleProps> = ({article}) => {
    let avatar= article?.avatar? article.avatar: "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png"
   let author = article?.created_by? article.created_by as IUser : null
          ...
          >
            ...
            >
              ...
              <SemiSpan color="text.muted" ml="12px">
                {author?.firstName ? `created by ${author.firstName}  on ${format(new Date(article.createdAt), "dd MMMM, yyyy")}`:
                 `created on ${format(new Date(article.createdAt), "dd MMMM, yyyy")}`}
                </SemiSpan>
            </Box>
          </FlexBox>
...
export default ArticleIntroCard;
